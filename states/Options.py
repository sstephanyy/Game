...
class Options(GameState):
    ...
    def update(self, surf=screen):
        ...
        self.options = [f'RESOLUÇÃO - {self.config_res}', f'MOSTRAR FPS: {bool2Switch(config.show_fps)}',
                       f'TELA CHEIA: {bool2Switch(config.set_fullscreen)}', 'APLICAR RESOLUÇÃO', 'VOLTAR']
        ...
        if event.type == KEYDOWN:
            ...
            if event.key in CONTROLS['START']:
                ...
                if self.selected == 0:
                    ...
                    print("\"Mostrar FPS\" não está disponível nesta versão")
                    config.show_fps = not config.show_fps
                elif self.selected == 2:
                    print("\"Tela cheia\" não está disponível nesta versão")
                    config.set_fullscreen = not config.set_fullscreen
                elif self.selected == 3:
                    if config.window_size != (1280, 720):
                        print("Configuração de resolução não disponível nesta versão, retorne para 720p")
